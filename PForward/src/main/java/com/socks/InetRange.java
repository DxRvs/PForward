...
public class InetRange implements Cloneable{
    ...
    public synchronized boolean remove(String s){
      Enumeration enums = all.elements();
      while(enums.hasMoreElements()){
        Object[] entry = (Object[]) enums.nextElement();
        if(s.equals(entry[0])){
...
/////////////////
    ...
    private boolean checkHostEnding(String host){
       Enumeration enums = end_names.elements();
       while(enums.hasMoreElements()){
          if(host.endsWith((String) enums.nextElement())) return true;
       }
       return false;
    }
