...
import com.socks.server.ServerAuthenticator;
...
public class ProxyServer implements Runnable {
	...
	private int runned_port = 0;
	private InetAddress runned_inetaddr = null;
	private String runned_message = "";
	...
	public int getRunnedPort() {
		return runned_port;
	}

	public String getRunnedInetAddr() {
		if(runned_inetaddr!=null){
		return runned_inetaddr.getHostAddress();
		}else{
			return "NULL";
		}
	}

	public String getRunnedMessage() {
		return runned_message;
	}
	...
	public void start(int port, int backlog, InetAddress localIP) {
		...
		try {
			...
			t.start();
			runned_inetaddr = ss.getInetAddress();
			runned_port = ss.getLocalPort();
		} catch (IOException ioe) {
			runned_message = ioe.getMessage();
		}
