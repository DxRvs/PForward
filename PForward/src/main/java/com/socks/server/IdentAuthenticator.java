...
import com.socks.InetRange;
import com.socks.ProxyMessage;
...
public class IdentAuthenticator extends ServerAuthenticatorNone{
  ...
  private int getRangeIndex(InetAddress ip){
     ...
     while(enums.hasMoreElements()){
       InetRange ir = (InetRange) enums.nextElement();
  ...
  private String userNames(int i){
    ...
    Enumeration enums = ((Hashtable)users.elementAt(i)).keys();
    if(!enums.hasMoreElements()) return "";
    String s = enums.nextElement().toString();
    while(enums.hasMoreElements())
       s += "; "+enums.nextElement();
